...
*/
void eval(char *cmdline) {
	/*char *argv[MAXARGS]; //this will allocate size MAXARGS definec above
		int bg = parseline(cmdline,argv); // 1 is bg job, 0 if fg job
		pid_t pid = jobs->pid; 		// Process id 
...
int is_builtin_cmd(char **argv) {
	if (strcmp("exit",argv[0]) == 0) {
		do_exit(); //exit program instead of continuing with non-built-in
	...
	else if (strcmp("killall",argv[0]) == 0) {
		do_killall(argv);
		return 1;
	}
...
void sigchld_handler(int sig) {
	...
	while ((pid = waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0) {
		...
		if (WIFEXITED(status)) { 
			removejob(jobs, pid); // Delete the child from the job list 
		}
		...
		// Stop signal for sigtstp
			...
			struct job_t* temp = getprocessid(jobs, pid);
			temp->state = ST;
		}
	}
}
